...
var audio = new Audio('/static/games/colorun/color.mp3');

$(document).ready(function () {
  if (window.innerHeight > window.innerWidth) {
    document.getElementById("back").style.opacity = "1";
    document.getElementById("back").style.zIndex = "99";
  }
});
...
class Game {
  ...
  showGrade(score) {
    ...
    if (score > 30) {
      ...
      return `Master! \nYou 've made ${score}% of your staking`
    }
    else if (score > 25){ 
      var amount = window.stakeAmount
      let status = "success"
      ...
      return `You made ${score}% of your staking`
    }
    else {
            var amount = window.stakeAmount
            let status = "failed"
      ...
      return `Sorry you lose!,Just ${5-score} more to start earning`
    }
  }
   ...
   */
  moveToStart() {
    let tip = new TimelineMax({ delay: 0 });
    ...
    tip
      ...
      .to(
        ".learn-to-play",
        1,
        { scale: 0, opacity: 0, ease: Elastic.easeOut.config(1.25, 5) },
        2
      );
...
$(document).on("touchstart", ".stick", function () {
...
$(document).on("touchstart", ".section-2 .bar", function () {
  color.changeColor($(this));
});
