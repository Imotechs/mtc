...
function gameOver(){
  ...
  instructionz.style.display = 'none'
  gameStatus = "gameOver";
  monster.sit();
  hero.hang();
  ...
  var topay = distance/3
  console.log('you ran',topay)
  var url = '/games/temple/runz/'
  if (topay>=500){
    ...
    let points = 'none'
          fetch(url,{
              ...
              body:JSON.stringify({"amount": amount,"status": status,"points":'none'})
          });
  }
...
function replay(){
  window.stakeAmount =prompt('stake to play')
...
var instructionz = document.getElementById('instruction')
